...
function MainSoundField() {
    ...
    return (<div className="w-[40vw] m-auto mt-[1vh] p-[5px] bg-mantle block relative">
        <SoundSearch setSongs={setSongs} />
        <ul className="m-[0.5vw] mt-[2vh] mb-[10vh] -z-9">
...
function PlaylistField() {
    ...
    useEffect(() => { fetch('http://127.0.0.1:8000/playlists').then(r => r.json()).then(d => setPlaylists(d)) }, [])

    return (
        <div className="w-[25vw] mr-[2.5vw] mt-[1vh]">
            ...
            <div className=''>
                ...
                <button onClick={() => {
                    if (canCreatePlaylist) {
                        ...
                        fetch('http://127.0.0.1:8000/playlist', { method: 'POST', headers: { 'Content-Type': 'application/json;charset=utf-8' }, body: JSON.stringify({ name: "new playlist", additional_data: "" }) })
...
function App() {
    return (
        <>
            <div className="flex">
                <div className="w-[25vw] ml-[2.5vw]"></div>
                <MainSoundField />
                <PlaylistField />
            </div >
            <MainPlayer />
